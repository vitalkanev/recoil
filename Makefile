...
recoil.so: recoilmagick.c recoil.c recoil.h
ifdef MAGICK_INCLUDE_PATH
	$(CC) $(CFLAGS) $(MAGICK_CFLAGS) -I"$(MAGICK_INCLUDE_PATH)" recoilmagick.c recoil.c \
...
clean:
	rm -f recoil2png recoil.so coder.xml.new recoil-mime.xml benchmark
...
recoil-mime.xml: recoil-types.xsl recoil-types.xml
	xsltproc -o $@ recoil-types.xsl recoil-types.xml

install-thumbnailer: recoil-mime.xml install-recoil2png
...
uninstall-thumbnailer: uninstall-recoil2png
...
.PHONY: all clean install uninstall install-recoil2png uninstall-recoil2png $(if $(CAN_INSTALL_MAGICK),install-magick uninstall-magick) install-thumbnailer uninstall-thumbnailer missing-examples
